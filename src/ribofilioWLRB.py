...
def plots(plot_flag,outName,N,coverage,all_bins,binsize, posmax, posmin,last_pos,gLength,ymin,ymax,ylogmin,ylogmax,bCovered,regfile):
    ...
    if plot_flag ==1: 
        ...
        plt.savefig(outName+"_"+str(binsize)+".NoLog.png", format='png')
        plt.clf()
        ...
        plt.savefig(outName+"_"+str(binsize)+".LogLog.png", format='png')
        plt.clf()
        ...
        plt.savefig(outName+"_"+str(binsize)+".LogLinear.png", format='png')
        plt.clf()
    ...
    plt.savefig(outName+"_"+str(binsize)+".WLR.png", format='png')
...
def main(): 
    ...
    if args.rna != "NULL": 
        if subset == "NULL":
            gbins1, last_pos, bCovered = profile(args.footprint,binsize, coverage, gLength,posmax, posmin)
            gbins2,_,_ = profile(args.rna,binsize, coverage, gLength,posmax, posmin) 
        else:
            gbins1, posmax, gLength, bCovered = run_subset(args.transcripts,args.footprint, subset, binsize)
            gbins2, posmax, gLength, bCovered = run_subset(args.transcripts,args.rna, subset, binsize)
            last_pos = posmax
    ...
    #-----------------------------------------
    plots(plot_flag,outName,N,coverage,gbins,binsize, posmax, posmin,last_pos,gLength,int(args.ymin),int(args.ymax),int(args.ylogmin),int(args.ylogmax),bCovered,args.regfile_name )
